...
sdata = sc.read_h5ad('/stanley/WangLab/Data/Analyzed/2022-09-05-Hu-Tissue/output/2022-11-20-Brain-RIBOmap-3mad-ct-final_region_label.h5ad')
idx_t = sdata.obs['replicate'] == 'rep2'
...
del sdata
output_path = f'/stanley/WangLab/atlas/Brain/09.TE_analysis/All_results_v8_{ctype_col}_RIBOmap_rep2_{percent_cutoff}'
...
for ctype_t in ctype_list:
    ...
    ### TE analysis
    if np.isin([f'{ctype_t}_RIBOmap_mean'],list(data_dict.keys())).sum() < 1:
        continue
...
for region_t in region_list:
    ...
    ### TE analysis
    if np.isin([f'{region_t}_RIBOmap_mean'],list(data_dict.keys())).sum() < 1:
        continue
