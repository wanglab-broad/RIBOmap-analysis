...
addpath('Z:\atlas\Brain\08.region\AP_histology-master')
addpath('Z:\atlas\Brain\08.region\npy-matlab-master\npy-matlab')
addpath('Z:\atlas\Brain\08.region\AP_histology-master\allenCCF_repo_functions')
addpath('Z:\atlas\Brain\08.region\AP_histology-master\other_function')
%% set batch id
label_file=[1,2];

all_file_name=dir('Z:\atlas\Brain\08.region\RIBOmap_rep23_region\*.tif');
...
slice_im_fn={};
for lengi=1:length(label_file)
    slice_im_dir = dir([im_path filesep filename{lengi} '.tif']);
...
%slice_size_max = [3000,3000];%max(cell2mat(cellfun(@size,slice_im,'uni',false)),[],1);
...
%slice_im_pad = slice_im_pad(:,:,2);  %% since the slice_im_pad have 3 layers

%% rotate for each dataset
im_aligned = zeros([slice_size_max(1),slice_size_max(2),slice_size_max(3),length(label_file)],class(slice_im_pad));
%im_aligned = zeros([3000,3000,length(label_file)],class(slice_im_pad));
for lengi=1:length(label_file)
    align_axis=load([slice_im_path filesep 'align_axis.mat']);
    align_axis=align_axis.align_axis;
    align_angle = squeeze(atan2d(diff(align_axis(:,1,:),[],1),diff(align_axis(:,2,:),[],1)));
    align_center = squeeze(mean(align_axis,1,'omitnan'));
    % Set target angle as the nearest multiple of 90
    target_angle = round(mean(align_angle,'omitnan')/90)*90;
    % Set target position as the average center of the reference lines
    if size(align_center,1)>1
        target_position = mean(align_center,2,'omitnan');
    else
...
for lengi=1:length(label_file)
    ...
    ccf_slice_fn_part = [slice_im_path filesep 'histology_ccf.mat'];
...
for lengi=1:length(label_file)
    ...
    atlas2histology_tform_all(1,lengi) = atlas2histology_tform(lengi); 
end
...
for ij=12:length(gui_data.slice_im)
...
%slice_size_max=[3000,3000,3];
slice_size_max=[slice_size_max(1),slice_size_max(2),slice_size_max(3)];
for lengi=1:length(label_file)
    annotation_image=gui_data.histology_aligned_av_slices{lengi};

    label_id=num2str(label_file(lengi));
    ...
    % rotate back
    annotation_image1 =imrotate(annotation_image,-angle_diff,'bilinear','crop');    
    % translate back
    annotation_image2= imtranslate(annotation_image1, [-x_diff,-y_diff]);

    im=imread(string(slice_im_fn{lengi}));

    index1=ceil((slice_size_max(1) - size(im,1))./2);
    index2=ceil((slice_size_max(2) - size(im,2))./2);
    if index1 == 0
        index1 = 1;
    end
    if index2 == 0
    ...
    annotation_image3=annotation_image2(index1:index1+size(im,1)-1,index2:index2+size(im,2)-1);


    imwrite(uint16(annotation_image3),[slice_im_path filesep 'ccf_',filename{lengi},'.tif']);
...
for lengi=1:length(label_file)
    annotation_image=gui_data.histology_aligned_av_slices{lengi};
    % ccf images
    imwrite(uint16(annotation_image),[slice_im_path filesep 'ccf_zf',filename{lengi},'.tif']);
    ccf_label = unique(annotation_image);
    % ccf labels
    ccf_label_table = table(ccf_label,'VariableNames',{'ccf_label'});
    writetable(ccf_label_table,fullfile(slice_im_path, strcat('ccf_labels_zf',filename{lengi},'.csv')),'Delimiter',',','QuoteStrings',false);
end
